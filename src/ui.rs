...
pub struct UI {
	stdout: Stdout, matcher: SkimMatcherV2,
	items: Vec<String>, display_items: Vec<String>, 
	num_of_items: usize,
	prompt: String, selected: usize, 
...
impl UI {
	pub fn new(items: &Vec<&str>) -> Self {
		...
		Self {
			stdout: stdout(), matcher: SkimMatcherV2::default(),
			items: items.iter().map(|&s| s.into()).collect(), 
			display_items: Vec::<String>::new(),
			num_of_items: 0,
			prompt: String::new(), selected: 0,
			start_index: 0, end_index: (h-1) as usize
		}
	}
							...
							match event.code {
								...
								KeyCode::Enter => {
									self.stdout
										.queue(LeaveAlternateScreen)?
										.queue(DisableMouseCapture)?;
									println!("{}", self.display_items[self.selected]);
									break;
								},
